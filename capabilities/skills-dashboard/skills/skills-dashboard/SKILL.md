...
description: >-
  Start, stop, or check a local Skills/Capabilities relationship dashboard
  through the skills-dashboard capability. Use when
...
仅当结果包含 `skills-dashboard` 时继续。独立使用本仓库时不需要 `wu`，可以直接调用
`capabilities/skills-dashboard/adapters/skills-dashboard`。
